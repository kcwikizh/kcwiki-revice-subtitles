<template>
  <div id="home" class="ui column centered stackable grid">
    <div class="column ui segment">
      ...
      <div class="three wide column" v-for="ship in shipList | filterBy query" v-if="ship.wiki_id && ship.id < 1500"><a v-link="'List/'+ship.id" class="ui tag teal label">{{ship.wiki_id}}.{{ ship.name }}</a></div>
      </div>
    </div>
  </div>
</template>
